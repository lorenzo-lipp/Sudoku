class Sudoku {
    ...
    hasOnlyOneSolution(puzzleString) {
    ...
    newGame() {
        let puzzleString = ".".repeat(81);

        for (let i = 1; i <= 9; i++) {
            puzzleString = this.write(puzzleString, Math.floor(Math.random() * 9 + 1), Math.floor(Math.random() * 9 + 1), i);
        }

        let solution = this.solve(puzzleString);
        let finalPuzzle = solution;
        let finalPuzzleArr = Array(finalPuzzle.length).fill(0).map((v, i) => i);
    
        for (let i = 0; i < finalPuzzle.length; i++) {
            let index = Math.floor(Math.random() * finalPuzzleArr.length);
            let strPosition = finalPuzzleArr[index];
            let puzzleWithoutCharacter = finalPuzzle.substring(0, strPosition) + "." + finalPuzzle.substring(strPosition + 1);
            if (this.hasOnlyOneSolution(puzzleWithoutCharacter)) finalPuzzle = puzzleWithoutCharacter;
            finalPuzzleArr.splice(index, 1);
        }
